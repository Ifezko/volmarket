...
The keeper selects the path with the `MOCK_VALIDATOR` flag (default `true`) plus the `TXLINE_PROGRAM_ID` it passes. The running demo uses the mock — not because the integration is incomplete, but because of epoch timing.
