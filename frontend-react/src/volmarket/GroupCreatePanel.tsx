...
export function GroupCreatePanel({
  ...
  seedCode?: string
  onCreate: (group: { name: string; visibility: 'Public' | 'Private'; joinMode: 'link' | 'invite' }) => void
  ...
  function handleCreate() {
    onCreate({ name: finalName, visibility, joinMode })
    setCreated(true)
        ...
        <div className="selrow" style={{ marginBottom: 12 }}>
          <div>
            ...
            <div className="s" style={{ color: 'var(--dim)' }}>
              {visibility} · {joinMode === 'link' ? 'Anyone with link can join' : 'Invite only'}
            </div>
          </div>
        </div>
